...
| `apple_calendar.py` | `is_accessible()` — detects whether `ical-guy` can read Apple Calendar on this machine (macOS ≥14, binary found, probe succeeds); `get_tomorrow_events(target_date, exclude_calendars, timezone_str)` — reads all events from all Apple Calendars via ical-guy, converts UTC to local timezone, returns sorted list in canonical event-dict shape. Never raises on accessibility failure — callers catch `RuntimeError` and fall back |
| `calendar_reader.py` | `get_msi_time_blocks()` — free/busy blocks only (freeBusyReader permission); `get_personal_events()` — full events with title, description, location. Both return tz-aware datetimes. Used only when Apple Calendar is unavailable |
...
| `sync_job.py` | `run_nightly_sync(app_ref)` — the full pipeline behind a `threading.Lock`; selects Apple Calendar or Google Calendar as the read source per run; `queue_run()` queues one pending run via `threading.Event`; `_show_popup()` / `_classify_unknown_blocks()` / `_classify_personal_events()` / `_prompt_unknown_locations()` handle the interactive osascript dialogs |
...
app.py (9 PM trigger or "Run now")
  └── sync_job.queue_run(app_ref)
        └── run_nightly_sync(app_ref)
              ...
              ├── apple_calendar.is_accessible()?
              │     ├── YES → apple_calendar.get_tomorrow_events()  [ical-guy / EventKit]
              │     │           └── on failure → rumps.notification + fallback to Google reads
              │     └── NO  → calendar_reader.get_msi_time_blocks()
...
**Apple Calendar is the preferred read source on macOS; Google Calendar is the fallback.**
When `ical-guy` is installed and Calendar permission is granted, all events from all Apple Calendars are read into a single unified pool and fed into the pipeline as `msi_blocks`. There is no work/personal distinction in the Apple path — classification dialogs handle unknown events just as in the Google path. Alarm writes always go to Google Calendar regardless of which read source was used. Falls back silently to Google Calendar reads when `ical-guy` is unavailable (not installed, macOS < 14, or permission denied) or when the read fails at runtime.
